...
import estacionamento.MatriculaRepetidaException;
import estacionamento.ParqueCheioException;
...
public class JanelaEntradaSimples extends JDialog {
	...
	private void processarEntrada() {
		...
		// caso corra bem
		try {
			parque.entrar( matricula );
			entreLbl.setIcon( verdeOn );
		// caso esteja cheio
		} catch (ParqueCheioException e) {
			entradaNegadaLbl.setIcon( vermelhoOn );
			cheioLbl.setIcon( vermelhoOn ); // este provavelmente não é preciso mas vamos testar
