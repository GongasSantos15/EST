...
public class Xadrez extends JFrame implements ActionListener {
	...
	public void iniciarJogo(){
	    ...
	    // TODO AFEITO criação e colocação dos peões
	    for (int i = 1; i <= 8; i++) {
	    ...
	    // TODO AFEITO criação e colocação das restantes peças brancas	    
	    ...
	    // TODO AFEITO criação e colocação das restantes peças pretas
	...
	private void desenharJogo(Graphics g) {
		...
		if( selecionada != null ) {
			if( destino != null ) {
				...
				g.setColor(selecionada.podeMover(destino) ? new Color( 50, 255, 50, 180 ) : new Color(255, 50, 50, 150 ));
		...
		if ( turno == Peca.BRANCAS ) {
			...
			g.fillOval( largTab + 10, largTab - 30, 20, 20 );
		}
		else {
	...
	private void pegarPeca(MouseEvent e) {
		...
		// TODO ZFEITO ver qual a peça a selecionar e se pode ser selecionada
