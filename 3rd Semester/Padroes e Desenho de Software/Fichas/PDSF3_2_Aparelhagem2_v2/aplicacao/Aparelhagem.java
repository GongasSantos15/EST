...
public class Aparelhagem extends JFrame implements BotaoRotativoListener {
	...
	/** variável que indica a cor do display inicial */
	private Color corDisplay = COR_OFF;
...
//	}
	...
	// Método que modifica a cor do ecrã quando o utilizador clica no botão
	@Override
	public void botaoComecaRodar(EventoBotaoRotativo e) {
		corDisplay = COR_ON;
		display.repaint();
		
	}
	
	// Método que verifica se os botões estão a ser rodados pelo utilizador, se sim incrementar o valor da frequência e/ou volume
	@Override
	public void botaoEstaRodar(EventoBotaoRotativo e) {
		int incremento = e.getIncremento();
		if( e.getSource() == btVolume ){
			alterarVolume( incremento );
		}
		if( e.getSource() == btFreq ){
			mudarFrequencia( incremento );
		}
	...
	// Método que modifica a cor do ecrã quando o utilizador larga o botão
	@Override
	public void botaoRodou(EventoBotaoRotativo e) {
		corDisplay = COR_OFF;
		display.repaint();
